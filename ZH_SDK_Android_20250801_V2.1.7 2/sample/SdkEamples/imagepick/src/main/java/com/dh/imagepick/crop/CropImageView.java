...
public class CropImageView extends AppCompatImageView {
    ...
    public void setMinWidth(int minWidth) {
        this.minWidth = Math.max(mFocusWidth,minWidth);
    }
    public void setMinHeight(int minHeight) {
        this.minHeight = Math.max(mFocusHeight,minHeight);
    }
    //TODO 计算最小比例
    ...
    private void fixScale() {
        ...
        float minScale = getScale(mRotatedImageWidth, mRotatedImageHeight, minWidth, minHeight, true);
...
//                bitmap = Bitmap.createScaledBitmap(bitmap, expectWidth, exceptHeight, true);
                bitmap = zoomBitmap(bitmap, expectWidth, exceptHeight);
    ...
    public void saveBitmapToFile(File folder, int expectWidth, int exceptHeight, boolean isSaveRectangle) {
        ...
        File saveFile = createFile(folder, "IMG_", ".png");
//        if (mStyle == Style.CIRCLE && !isSaveRectangle) {
