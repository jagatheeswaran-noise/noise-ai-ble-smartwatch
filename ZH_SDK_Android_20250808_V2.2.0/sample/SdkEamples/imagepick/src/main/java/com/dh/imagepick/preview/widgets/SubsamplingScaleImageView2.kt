...
import com.dh.imagepick.preview.ImageViewerViewModel
import com.dh.imagepick.preview.utils.Config
import com.dh.imagepick.preview.utils.ViewModelUtils.provideViewModel
    ...
    private fun handleDispatchTouchEvent(event: MotionEvent?) {
        when (event?.actionMasked) {
            ...
            MotionEvent.ACTION_MOVE -> {
                val range = RectF()
                getPanRemaining(range)
                if (singleTouch && (scale == initScale || range.top == 0f)) {
